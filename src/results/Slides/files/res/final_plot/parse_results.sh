...
gnuplot << EOF
    ...
    set ylabel "Time"
    set title "Time"
    plot    "${rec_cp}" using 2:xtic(1) lt 3 title "rec gcc-cilkplus" with lines, \
         "${tiled_rcp}" using 2:xtic(1) lt 4 title "tiled rec_for gcc-cilkplus" with lines, \
        "${tiled_cfcp}" using 2:xtic(1) lt 5 title "tiled cilk_for gcc-cilkplus" with lines
    unset ylabel
    ...
    plot    "${rec_cp_spd}" using 2:xtic(1) lt 3 notitle with points, \
            "${rec_cp_spd}" using 2:xtic(1) lt 3 title "rec gcc-cilkplus" with lines, \
         "${tiled_rcp_spd}" using 2:xtic(1) lt 4 notitle with points, \
         "${tiled_rcp_spd}" using 2:xtic(1) lt 4 title "tiled rec_for gcc-cilkplus" with lines, \
        "${tiled_cfcp_spd}" using 2:xtic(1) lt 5 notitle with points, \
        "${tiled_cfcp_spd}" using 2:xtic(1) lt 5 title "tiled cilk_for gcc-cilkplus" with lines
    unset multiplot
    set key 
EOF
}
...
plotot "final_plot.png" sandman_rec_time.res sandman_tiled_cilk_for.res sandman_tiled_cilkplus_rec_for_sweep.res sandman_rec_speedup.res sandman_tiled_cilk_for_speedup.res sandman_tiled_cilkplus_rec_for_sweep_speedup.res
